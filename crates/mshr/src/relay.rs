...
//! can host the relay infrastructure that mshr's swarm/NAT-traversal
//! lanes depend on.
//!
...
//! `app/yah/desktop/` and the yubaba crate. mshr only owns the
//! "how do I host the relay" half.
//!
//! ## The `crate::endpoint::Acceptor` hook does not apply here
//!
//! [`crate::Acceptor`] (R593-F3) gates `crate::Endpoint::accept_dispatch`'s
//! ALPN-dispatched application connections — it isn't wired into
//! [`Server`]. Two reasons this is out of scope rather than an oversight:
//!
//! - **No per-connection application dispatch to gate.** The relay's job
//!   is a bytes-agnostic proxy: WebSocket-tunnel already-encrypted QUIC
//!   packets between two *other* endpoints (or serve QUIC address
//!   discovery / pkarr publish). There is no ALPN handler on the far side
//!   of `Server` for a deny decision to preempt — accepting or denying at
//!   this layer would just be "should this proxy exist at all," a
//!   capacity/abuse concern, not the identity-gate this hook is for.
//! - **No accept loop we own.** `Server::builder().start()` hands the
//!   fully-assembled `ServerConfig` to `iroh_relay::server::Server::spawn`,
//!   which owns its own internal connection handling; mshr doesn't see
//!   individual relay-client connections to hook.
//!
//! If a relay-level admission control is ever needed (rate-limiting,
//! denylisting abusive NodeIds at the WebSocket/QUIC-discovery layer),
//! it's a new, separate concern from the identity gate this ticket adds —
//! most likely upstream in `iroh_relay`, not a reuse of `Acceptor`.
