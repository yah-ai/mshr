...
[![crates.io](https://img.shields.io/crates/v/mshr.svg)](https://crates.io/crates/mshr)
[![docs.rs](https://docs.rs/mshr/badge.svg)](https://docs.rs/mshr)
...
`mshr` is the single upgrade point for those concerns. Consumers import
`mshr::Endpoint`, not `iroh::Endpoint` — so when iroh ships a breaking
...
This is the seam that lets `mshr` stay agnostic of your application's
...
```rust,ignore
use mshr::{Endpoint, RelayMap, RelayUrl};
...
`mshr` follows iroh's `1.0.0-rc.0` line. The crate is versioned in lockstep
with the yah release train (currently `0.8.x`); the public API is expected to
stabilize alongside iroh's 1.0 release.
...
`mshr` re-exports the iroh types you'll need at the API boundary so
consumers don't have to take a direct iroh dep:

- `mshr::EndpointId` (alias `NodeId`), `EndpointAddr` (alias `NodeAddr`)
- `mshr::SecretKey`
- `mshr::RelayMap`, `RelayMode`, `RelayUrl`
