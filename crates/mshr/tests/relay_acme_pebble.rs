...
//! testing ACME server. Exercises [`mshr::relay::Server`]'s
...
//! host. The test stays out of the default `cargo test` run so mshr
...
use mshr::{relay, Endpoint, Keypair};
...
async fn relay_acme_pebble_round_trip() {
    let _ = tracing_subscriber::fmt()
        .with_env_filter("info,mshr=debug,iroh=info,iroh_relay=info,tokio_rustls_acme=debug")
        .with_test_writer()
        .try_init();
