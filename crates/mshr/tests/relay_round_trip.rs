//! End-to-end test for [`mshr::relay::Server`] (R105-F4 acceptance).
//!
//! Spawns an in-process relay with a self-signed cert, binds two
//! `mshr::Endpoint`s that route through it via
...
async fn relay_round_trip_self_signed() {
    let _ = tracing_subscriber::fmt()
        .with_env_filter("warn,mshr=info,iroh=info,iroh_relay=info")
        .with_test_writer()
        .try_init();
